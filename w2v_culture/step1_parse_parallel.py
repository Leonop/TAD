...
if __name__ == "__main__":
    with CoreNLPClient(
        ...
        in_file_index = file_util.file_to_list(
            Path(gl.DATA_FOLDER_W2V, "input", "document_ids.txt")
        )
        out_file = Path(
            gl.DATA_FOLDER_W2V, "processed", "parsed", "documents_sent.txt"
        )
        output_index_file = Path(
            gl.DATA_FOLDER_W2V, "processed", "parsed", "document_sent_ids.txt"
        )
        process_largefile(
            input_file=in_file,
            ...
            chunk_size=gl.PARSE_CHUNK_SIZE,
        )
